...
def host_choice(doors: list, prize_index:int, player_index:int) -> int:
    ...
    choices = [i for i in range(len(doors)) if i != prize_index and i != player_index]
...
def get_alt_index(doors: list, player_index:int, host_index:int) -> int:
    ...
    all__alt_index = next(i for i in range(len(doors)) if i != player_index and i != host_index)
    return all__alt_index
